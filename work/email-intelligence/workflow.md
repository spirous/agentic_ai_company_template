...
**Public tier** (`company/style/`): style guide + patterns — safe for GitHub, no names.
**Private tier** (`knowledge/contacts/`): per-contact context — local only.
**Approved drafts** (`company/style/approved/`): gitignored — may contain real names.

## How the knowledge base grows
- Every `[a]pprove` saves the draft to `company/style/approved/YYYY-MM-DD_NN.md`
